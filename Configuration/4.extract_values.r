...
vars<-c("precipmon_av", "tempmonmax_abs", "tempmonmin_abs", "tempmon_av")
...
for (i in c(1:nrow(ages))){
  ...
  for (v in vars){
    ...
    rm("r_final")
    if (v=="precipmon_av"){
      r_final<-sum(r_sub, na.rm=T)*30
    }
    if (v=="tempmonmax_abs"){
      r_final<-max(r_sub, na.rm=T)
    }
    if (v=="tempmonmin_abs"){
      r_final<-min(r_sub, na.rm=T)
    }
    if (v=="tempmon_av"){
      r_final<-mean(r_sub, na.rm=T)
    }
...
if (F){
  ...
  rtmax<-rast("/media/huijieqiao/Butterfly/GABI/Data/Raster/Fine.1x1/tasmax.tif")
  rtmin<-rast("/media/huijieqiao/Butterfly/GABI/Data/Raster/Fine.1x1/tasmin.tif")
  rtmean<-rast("/media/huijieqiao/Butterfly/GABI/Data/Raster/Fine.1x1/tasmean.tif")
  
  rrr<-c(rtmax[[1]], rtmin[[1]], rtmean[[1]])
  plot(rrr)
}
