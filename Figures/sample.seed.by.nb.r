...
if (T){
  for (threshold in seq(from=90, to=100, by=1)){
    ...
    names(full.quantile)<-gsub("%", "", names(full.quantile))
    quantile_index<-which(names(full.quantile)==as.character(threshold))
    
   
    p<-ggplot(N_species.all)+
      geom_histogram(aes(x=N_SPECIES), binwidth=1)+
      geom_vline(aes(xintercept=full.quantile[quantile_index]), linetype=2)+
      scale_x_sqrt(breaks=c(100, 1000, ceiling(full.quantile[quantile_index]), 2500, 5000, 7500, 10000))+
      scale_y_sqrt()+
      theme_minimal() +
      labs(
        x = "Number of species",
    ...
    p
    ggsave(p, filename=sprintf("../Figures/Outliers/Outlier.distribution.%d.pdf", threshold), width=6, height=3)
    ggsave(p, filename=sprintf("../Figures/Outliers/Outlier.distribution.%d.png", threshold), width=6, height=3, bg="white")
    ...
    outliers<-unique(N_species[(N_SPECIES>full.quantile[quantile_index])])
    ...
    to.doc(dt.outliers, sprintf("Number of outliers per combination, threshold: %d, %.2f",
                                threshold,
                                full.quantile[quantile_index]), 
           sprintf("../Figures/Outliers/outliers.%d.docx",threshold),
           digits = 0)
    ...
    p
    ggsave(p, filename=sprintf("../Figures/Outliers/Outlier.details.%d.pdf", threshold), width=6, height=6)
    ggsave(p, filename=sprintf("../Figures/Outliers/Outlier.details.%d.png", threshold), width=6, height=6, 
           bg="white")
    ...
    #ittem<-all_ramdom_seeds_df[seed_id==11625 & nb=="BIG" & da=="GOOD"][1]
    #ittem$rep<-3
    #all_ramdom_seeds_df<-rbindlist(list(all_ramdom_seeds_df, ittem))
    all_ramdom_seeds_df[N_SPECIES>5000]
    ...
    saveRDS(all_ramdom_seeds_df, sprintf("../Data/Tables/random.seeds.threshold.by.nb.distance.%d.rda", threshold))
  }
}
...
fwrite(N_times, 
       "../Figures/Seeds/picked.in.boot.csv")
