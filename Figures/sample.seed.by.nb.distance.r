...
seed_pool.sa<-seed_pool[continent=="South America"]
seed_pool.na<-seed_pool[continent=="North America"]

dist.bins<-seq(from=min(seed_pool$min.dist), to=max(seed_pool$min.dist), by=2)
...
for (rep in c(1:100)){
  ...
  for (nb.str in c("MODERATE-MODERATE", "BIG-BIG", 
    ...
    sampled.sa.list<-list()
    for (dist in dist.bins){
      n<-nrow(ramdom_seeds.na[between(min.dist, dist, dist+1)])
      if (n==0){
        next()
      }
      item.sa<-seed_pool.sa[between(min.dist, dist, dist+1)]
      replace<-ifelse(n>nrow(item.sa), TRUE, FALSE)
      sampled.sa<-item.sa[sample(nrow(item.sa), n, replace=replace)]
      sampled.sa.list[[length(sampled.sa.list)+1]]<-sampled.sa
    }
    ramdom_seeds.sa<-rbindlist(sampled.sa.list)
    ramdom_seeds<-rbindlist(list(ramdom_seeds.na, ramdom_seeds.sa), use.names = T)
    if (F){
