...
if (F){
  ...
  all_delta_Species<-all_delta_Species[, .(south.america=mean(south.america), 
                                           sd.south.america=sd(south.america),
                                           north.america=mean(north.america), 
                                           sd.north.america=sd(north.america)),
                                       by=list(NB, year, seed_continent)]
  
  saveRDS(all_delta_Species, "../Data/Tables/100k.speciation.years/all.N_Species.rda")
  
  delta_Species.origin<-readRDS("../Data/Tables/100k.speciation.years/delta_Species.origin.rda")
  type_N.origin<-readRDS("../Data/Tables/100k.speciation.years/type_N.origin.rda")
  
  all_delta_Species.origin<-list()
  for (rrrr in c(1:max(delta_Species.origin$rep))){
    item<-delta_Species.origin[rep==rrrr]
    prev_y<-item[year==-1800]
    print(paste(rrrr))
    for (y in c(-1799:0)){
      ...
      item_y$south.america<-item_y$south.america+item_y$c.south.america
      item_y$north.america<-item_y$north.america+item_y$c.north.america
      all_delta_Species.origin[[length(all_delta_Species.origin)+1]]<-item_y
      prev_y<-item_y[, c("NB", "year", "rep", "south.america", "north.america", "origin_continent")]
    }
  }
  all_delta_Species.origin<-rbindlist(all_delta_Species.origin)
  all_delta_Species.origin<-all_delta_Species.origin[, .(south.america=mean(south.america), 
                                           sd.south.america=sd(south.america),
                                           north.america=mean(north.america), 
                                           sd.north.america=sd(north.america)),
                                       by=list(NB, year, origin_continent)]
  
  saveRDS(all_delta_Species.origin, "../Data/Tables/100k.speciation.years/all.N_Species.origin.rda")
...
N.species$NB.Label<-factor(N.species$NB, 
...
p<-ggplot(N.species[year>=burn_in])+
  ...
  facet_grid(continent~NB.Label, scale="free")+
  theme_bw()
p
ggsave(p, filename="../Figures/N.Events/N.Species.png", width=12, height=6)

p<-ggplot(N.species[year>=burn_in & seed_continent!=continent])+
  geom_ribbon(aes(x=year, ymin=N-SD, ymax=N+SD, fill=seed_continent), color="lightgrey", alpha=0.3)+
  geom_line(aes(x=year, y=N, color=seed_continent))+
  facet_wrap(~NB.Label, scale="free")+
  theme_bw()
p
ggsave(p, filename="../Figures/N.Events/N.Species.exotic.png", width=10, height=6)

p<-ggplot(N.species.all[year>=burn_in & seed_continent!=continent])+
  geom_ribbon(aes(x=year, ymin=N-SD, ymax=N+SD, fill=seed_continent), color="lightgrey", alpha=0.3)+
  geom_line(aes(x=year, y=N, color=seed_continent))+
  theme_bw()
p
ggsave(p, filename="../Figures/N.Events/N.Species.all.exotic.png", width=10, height=6)


all_delta_Species.origin<-readRDS("../Data/Tables/100k.speciation.years/all.N_Species.origin.rda")


north<-all_delta_Species.origin[, c("NB", "year", "north.america", "sd.north.america", "origin_continent")]
colnames(north)[c(3,4)]<-c("N", "SD")
north$continent<-"North America"

south<-all_delta_Species.origin[, c("NB", "year", "south.america", "sd.south.america", "origin_continent")]
colnames(south)[c(3,4)]<-c("N", "SD")
south$continent<-"South America"

N.species.origin<-rbindlist(list(north, south))

N.species.origin$NB.Label<-factor(N.species.origin$NB, 
                           levels=c("BROAD-BROAD", "BIG-BIG", "MODERATE-MODERATE", "NARROW-NARROW"), 
                           labels=c("BROAD", "BIG", "MODERATE", "NARROW"))
p<-ggplot(N.species.origin[year>=burn_in])+
  geom_ribbon(aes(x=year, ymin=N-SD, ymax=N+SD, fill=origin_continent), color="lightgrey", alpha=0.3)+
  geom_line(aes(x=year, y=N, color=origin_continent))+
  facet_grid(continent~NB.Label, scale="free")+
  theme_bw()
p
ggsave(p, filename="../Figures/N.Events/N.Species.origin.png", width=12, height=6)

N.species.all.origin<-N.species.origin[, .(N=sum(N), SD=mean(SD)),
                         by=list(year, origin_continent, continent)]
p<-ggplot(N.species.all.origin[year>=burn_in])+
  geom_ribbon(aes(x=year, ymin=N-SD, ymax=N+SD, fill=origin_continent), color="lightgrey", alpha=0.3)+
  geom_line(aes(x=year, y=N, color=origin_continent))+
  facet_wrap(~continent)+
  theme_bw()
p
ggsave(p, filename="../Figures/N.Events/N.Species.all.origin.png", width=10, height=6)

p<-ggplot(N.species.origin[year>=burn_in & origin_continent!=continent])+
  geom_ribbon(aes(x=year, ymin=N-SD, ymax=N+SD, fill=origin_continent), color="lightgrey", alpha=0.3)+
  geom_line(aes(x=year, y=N, color=origin_continent))+
  facet_wrap(~NB.Label, scale="free")+
  theme_bw()
p
ggsave(p, filename="../Figures/N.Events/N.Species.exotic.origin.png", width=10, height=6)

p<-ggplot(N.species.all.origin[year>=burn_in & origin_continent!=continent])+
  geom_ribbon(aes(x=year, ymin=N-SD, ymax=N+SD, fill=origin_continent), color="lightgrey", alpha=0.3)+
  geom_line(aes(x=year, y=N, color=origin_continent))+
  theme_bw()
p
ggsave(p, filename="../Figures/N.Events/N.Species.all.exotic.origin.png", width=10, height=6)
...
N.type$NB.Label<-factor(N.type$NB, 
                        levels=c("BROAD-BROAD", "BIG-BIG", "MODERATE-MODERATE", "NARROW-NARROW"), 
                        labels=c("BROAD", "BIG", "MODERATE", "NARROW"))

p<-ggplot(N.type[year>=burn_in])+
  geom_ribbon(aes(x=year, ymin=abs(N)-SD, ymax=abs(N)+SD, fill=seed_continent), 
              color="lightgrey", alpha=0.3)+
  geom_line(aes(x=year, y=abs(N), color=seed_continent))+
  facet_grid2(type~NB.Label+continent, independent="y", scale="free")+
  scale_y_sqrt()+
  theme_bw()
p
ggsave(p, filename="../Figures/N.Events/N.Type.png", width=16, height=8)

N.type.all<-N.type[,.(N=sum(N), SD=mean(SD)),
                   by=list(year, type, seed_continent, continent)]

p<-ggplot(N.type.all[year>=burn_in])+
  geom_ribbon(aes(x=year, ymin=abs(N)-SD, ymax=abs(N)+SD, fill=seed_continent), 
              color="lightgrey", alpha=0.3)+
  geom_line(aes(x=year, y=abs(N), color=seed_continent))+
  facet_grid2(type~continent, independent="y", scale="free")+
  scale_y_sqrt()+
  theme_bw()
p
ggsave(p, filename="../Figures/N.Events/N.Type.all.png", width=16, height=8)
